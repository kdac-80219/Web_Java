...
public class UserDao extends Dao implements UserInterfaceDao {
	...
	public int save(UserPojo u) throws Exception {
		String sql = "INSERT INTO users(id, first_name, last_name, email,password, mobile, birth) VALUES(default, ?, ?, ?, ?, ?, ?)";
		try(PreparedStatement stmt = con.prepareStatement(sql)) {
			...
			stmt.setDate(6, java.sql.Date.valueOf(u.getbDate()));
		...
		try(PreparedStatement stmt = con.prepareStatement(sql)) {
			...
			try(ResultSet rs = stmt.executeQuery()) {
				while(rs.next()) {
					...
					return new UserPojo(id, fname, lname,email,password, mobile,date);
				}
			} // rs.close();
		} // stmt.close();
		return null;
